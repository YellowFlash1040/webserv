...
# Extract the color value from the input data
color=$(echo "$input" | sed -n 's/.*color=\([^&]*\).*/\1/p')

# Decode URL encoding (replace + with spaces and %xx with their ASCII character)
...
# Generate the HTML page with the selected background color
cat << EOF
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        ...
        <form action="/cgi-bin/display_color.sh" method="post">

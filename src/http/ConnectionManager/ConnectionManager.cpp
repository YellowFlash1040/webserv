...
void ConnectionManager::genRespsForReadyReqs(int clientId, const NetworkEndpoint& endpoint)
{
	(void)endpoint;
	...
	while (clientState.hasCompleteRawRequest())
	{
		...
		RequestContext ctx = m_config.createRequestContext(endpoint, reqData.getHeader("Host"), reqData.uri);
		printReqContext(ctx);
		...
		if (rawReq.isBadRequest())
		{
			...
			if (itErr != ctx.error_pages.end() && !itErr->second.empty())
			{
				...
				RequestContext errorCtx = m_config.createRequestContext(endpoint, reqData.getHeader("Host"), errorPageUri);

				if (std::find(errorCtx.allowed_methods.begin(),
							  errorCtx.allowed_methods.end(),
							  HttpMethod::GET) == errorCtx.allowed_methods.end())
				{
		...
		if (rawResp.isInternalRedirect() && status_code != HttpStatusCode::OK)
		{
			...
			else
			{
				...
				if (it404 != ctx.error_pages.end() && !it404->second.empty())
				{
					...
					std::cout << "[DEBUG] Using fallback 404 page: " << errorPageUri << "\n";
				}
				else
					std::cout << "[DEBUG] No fallback 404 page found, will use default error page\n";
			}
			...
			if (!errorPageUri.empty())
			{
				...
				if (errorResp.isInternalRedirect())
				{
					...
					errorResp.generateDefaultErrorPage(status_code); // fallback body
				}
			...
			else
			{
				...
				std::cout << "[DEBUG] Will perform internal redirect to: " << errorPageUri << "\n";

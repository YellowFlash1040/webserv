...
bool ConnectionManager::clientSentClose(int clientId) const
{
	...
	const ParsedRequest& req = it->second.getReqObj();
...
const ParsedRequest& ConnectionManager::getRequest(int clientId) const
{
	...
	const ParsedRequest& req = it->second.getReqObj();
	std::cout << "getRequest(): method=" << req.getMethod()
		<< " uri=" << req.getUri()
...
{	
	const std::string& buf = clientState.getRlAndHeadersBuffer();
	...
	const std::string& headers = clientState.getRlAndHeadersBuffer();

...
namespace BodyParser
{
	...
	size_t parseChunkSize(const std::string& buffer)
	{
		...
		if (pos == std::string::npos)
			throw std::runtime_error("Incomplete chunk size");

		std::string hex = buffer.substr(0, pos);
		std::stringstream ss;
		ss << std::hex << hex;
...
)
{
	...
	)
	{
		...
		while (pos < chunkedBuffer.size())
		{
			...
			DBG("[decodeChunkedBody]: chunkDataStart = " << chunkDataStart);

			size_t chunkDataEnd = chunkDataStart + chunkSize;
			DBG("[decodeChunkedBody]: chunkDataEnd = " << chunkDataEnd);
			...
			if (chunkSize > 0)
			{
				std::string chunkData = chunkedBuffer.substr(chunkDataStart, chunkSize);
				decoded += chunkData;
				DBG("[decodeChunkedBody]: Appended chunkData |" << chunkData 
					<< "|, decoded.size = " << decoded.size());

				pos = chunkDataEnd + 2; // skip chunkData + trailing \r\n
				DBG("[decodeChunkedBody]: skipped chunkData + chunkTrailer, pos = " << pos);
			}
			else
			{
				...
				DBG("[decodeChunkedBody]: zeroChunkEnd = " << zeroChunkEnd);

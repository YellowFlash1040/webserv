...
void RawRequest::separateHeadersFromBody()
{
    ...
    try
    {
        _uri = UriUtils::normalizePath(
            _uri); // validate path AFTER leftovers are handled
    }
...
void RawRequest::addHeader(const std::string& name, const std::string& value)
{
    ...
    throw std::runtime_error("Header conflict: " + name + " has values ["
                             + it->second + "] and [" + value + "]");
}
...
std::string RawRequest::extractHost(const std::string& hostHeader) const
{
    ...
    return (portPos != std::string::npos) ? hostHeader.substr(0, portPos)
                                          : hostHeader;
}
...
void RawRequest::appendToChunkedBuffer(const std::string& data)
{
    ...
    DBG("[appendToChunkedBuffer] After append, _chunkedBuffer size = "
        << _chunkedBuffer.size());
}

std::string RawRequest::decodeChunkedBody(size_t& bytesProcessed)
{
    DBG("[decodeChunkedBody]:" << " START: _chunkedBuffer = |" << _chunkedBuffer
                               << "|, _chunkedBuffer size = "
                               << _chunkedBuffer.size());
    ...
    while (pos < _chunkedBuffer.size())
    {
        ...
        {
            DBG("[decodeChunkedBody]: Incomplete chunkHeaderLine, waiting for "
                "more data");
        ...
        {
            DBG("[decodeChunkedBody]: Invalid chunk size |"
                << chunkSizeStr << "|, throwing exception");
            throw std::runtime_error("Invalid chunk size in chunked body");
        }

        DBG("[decodeChunkedBody]: Found the chunkHeaderLine |"
            << chunkHeaderLine << "|, so chunkSize = " << chunkSize
            << " bytes");

        size_t chunkDataStart = chunkLineEnd + 2; // skip \r\n
        DBG("[decodeChunkedBody]: Skipped \\r\\n from chunkLineEnd at pos "
            << chunkLineEnd << ". chunkDataStart pos is " << chunkDataStart);
        ...
        {
            DBG("[decodeChunkedBody]: Incomplete chunkData, waiting for more "
                "data");
            break; // wait for more data
        }
        ...
        if (chunkSize > 0)
        {
            ...
            decoded += chunkData;
            DBG("[decodeChunkedBody]: Appended chunkData |"
                << chunkData << "|, to decoded. decoded is now |" << decoded
                << "|, decoded.size = " << decoded.size());

            pos = chunkDataEnd + 2; // skip chunkData + trailing \r\n
            DBG("[decodeChunkedBody]: skipped chunkData + chunkTrailer, pos is "
                << pos);
        }
        else
        {
            ...
            DBG("[decodeChunkedBody]: zeroChunkEnd is set to 2 bytes after "
                "chunkDataStart ("
                ...
                && _chunkedBuffer[chunkDataStart + 1] == '\n')
            {
                ...
                DBG("[decodeChunkedBody]: Zero-size chunk fully consumed, pos "
                    "= "
                    << pos);
                break;
            }
            ...
            {
                DBG("[decodeChunkedBody]: waiting for final CRLF after zero "
                    "chunk");
                break; // wait for more data
            }
    ...
    bytesProcessed = pos;
    DBG("[decodeChunkedBody]: END: decoded is |"
        << decoded << "| with size = " << decoded.size()
...
{
    DBG("[consumeTempBuffer]: _tempBuffer before: |"
        << _tempBuffer << "|, size = " << _tempBuffer.size());
    ...
    DBG("[consumeTempBuffer]: after consuming: |"
        << _tempBuffer << "|, size = " << _tempBuffer.size());
}
...
void RawRequest::parseSizedBody(const std::string& data)
{
    ...
    DBG("[appendBodyBytes]: remaining bytes of content to append = "
        << remaining);
...
void RawRequest::parseChunkedBody()
{
    ...
    DBG("[appendBodyBytes]: appended _chunkedBuffer with data from "
        "_tempBuffer, _chunkedBuffer size = "
        << _chunkedBuffer.size());
    ...
    DBG("[appendBodyBytes]: decoded chunked body appended, decoded size = "
        << decoded.size());

    // remove processed bytes from _chunkedBuffer
    setChunkedBuffer(_chunkedBuffer.substr(bytesProcessed));
    DBG("[appendBodyBytes]: _chunkedBuffer resized after processing, new size "
        "= "
        << _chunkedBuffer.size());
    ...
    if (_terminatingZeroMet)
    {
        ...
        DBG("[appendBodyBytes]: Terminating zero chunk found, body done set, "
            "tempBuffer updated, _chunkedBuffer cleared");
    }
    else
    {
        ...
        DBG("[appendBodyBytes]: Terminating zero not found, leftovers moved to "
            "tempBuffer");
    }
}
...
size_t RawRequest::remainingConLen() const
{
    return static_cast<size_t>(getContentLengthValue()) > _conLenBuffer.size()
               ? static_cast<size_t>(getContentLengthValue())
                     - _conLenBuffer.size()
               : 0;
}
...
const std::unordered_map<std::string, std::string>& RawRequest::getHeaders()
    const
{
    return _headers;
}
...
{
    DBG("[setChunkedBuffer]: old size of _chunkedBuffer = "
        << _chunkedBuffer.size());

    _chunkedBuffer = std::move(newBuffer);

    DBG("[setChunkedBuffer]: new size of _chunkedBuffer = "
        << _chunkedBuffer.size() << ", content = |" << _chunkedBuffer << "|");
}

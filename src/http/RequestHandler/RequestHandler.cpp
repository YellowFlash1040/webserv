                            ...
                            RawResponse& redirResp)
{
    if (!FileUtils::existsAndIsFile(ctx.resolved_path)
        || access(ctx.resolved_path.c_str(), R_OK) != 0)
    {
...
RawResponse handleSingleRequest(const RawRequest& rawReq,
                                ...
                                const Config& config)
{
    RequestContext ctx = config.createRequestContext(endpoint, rawReq.getHost(),
                                                     rawReq.getUri());
    ...
    {
        std::string newUri = curRawResp.getErrorPageUri(

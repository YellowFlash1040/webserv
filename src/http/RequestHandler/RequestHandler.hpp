...
public:
    ...
    void processRequest(RawRequest& rawReq, Client& client, RequestContext& ctx);
    std::string getErrorPageUri(const RequestContext& ctx, HttpStatusCode status) const;
    void enqueueErrorResponse(RequestContext& ctx, HttpStatusCode status);
...
private:
    ...
    // теперь все методы принимают Client& вместо NetworkEndpoint&
    void processGet(RequestData& req, Client& client, RequestContext& ctx, RawResponse& resp);
    void processPost(RequestData& req, Client& client, RequestContext& ctx, RawResponse& resp);
    void processDelete(RequestData& req, Client& client, RequestContext& ctx, RawResponse& resp);
    ...
    void serveBadRequest(RawRequest& rawReq, Client& client, RequestContext& ctx);

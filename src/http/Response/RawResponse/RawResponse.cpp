...
RawResponse::RawResponse(const RequestData& req, const RequestContext& ctx)
	: _req(req), _ctx(ctx), _body(""), _isInternalRedirect(false)
{
...
const std::string& RawResponse::getBody() const { return _body; }
...
bool RawResponse::isMethodAllowed(HttpMethod method, const std::vector<HttpMethod>& allowed_methods)
{
...
void RawResponse::handleStatic()
{
	...
	if (isDir)
	{
		...
		std::cout << "[handleStatic] Autoindex disabled, directory without index file\n";
...
void RawResponse::generateDefaultErrorPage(HttpStatusCode code)
{
	...
	std::cout << "[generateDefaultErrorPage] Default error page generated for code "

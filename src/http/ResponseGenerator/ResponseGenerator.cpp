                     ...
                     const std::vector<HttpMethod>& allowed_methods)
{
    ...
    {
        DBG("[isMethodAllowed] Comparing with allowed method: "
            << httpMethodToString(allowedMethod));
...
void processGet(RequestData& req, const Client& client,
                const RequestContext& ctx, RawResponse& rawResp,
                CgiRequestResult& cgiResult)
{
    (void)client;
    DBG("[processGet] Processing request for resolved path: "
        << ctx.resolved_path);
    ...
    if (!ctx.cgi_pass.empty())
    {
        ...
        std::string interpreter
            = getCgiPathFromUri(req.uri, ctx.cgi_pass, status);

        DBG("[processGet] CGI interpreter path: \""
            << interpreter << "\", status: " << static_cast<int>(status));
        ...
        if (status == HttpStatusCode::OK)
        {
            ...
            DBG("[processGet] interpreter detected, checking file "
                << requestedScript << " for validity...");
    ...
    try
    {
        if (FileUtils::existsAndIsFile(ctx.resolved_path))
        {
            ...
            DBG("[processGet] Served static file with status: "
                << static_cast<int>(rawResp.getStatusCode())
    ...
    try
    {
        if (FileUtils::existsAndIsDirectory(ctx.resolved_path))
        {
            ...
            std::string indexPath = FileUtils::getFirstValidIndexFile(
                ctx.resolved_path, ctx.index_files);
            if (!indexPath.empty())
            {
            ...
            {
                DBG("[processGet] No index file found, generating "
                    "autoindex...");
    ...
    {
        DBG("[processGet] Exception serving directory or generating autoindex: "
            << e.what());
        rawResp.addErrorDetails(ctx, HttpStatusCode::InternalServerError);
        return;
    }
...
std::string getCgiPathFromUri(
    const std::string& uri, const std::map<std::string, std::string>& cgi_pass,
    HttpStatusCode& outStatus)
{
    ...
    {
        DBG("[getCgiPathFromUri] Checking pair: key=\""
            << entry.first << "\", interpreter=\"" << entry.second << "\"");
        ...
        {
            DBG("[getCgiPathFromUri] Interpreter is not a regular executable "
                "-> BadGateway");
            outStatus = HttpStatusCode::BadGateway;
            return "";
        }
...
bool checkScriptValidity(const std::string& scriptPath, RawResponse& rawResp,
                         const RequestContext& ctx)
{
    ...
    {
        DBG("[checkScriptValidity] Script exists but is not executable: "
            << scriptPath);
        rawResp.addErrorDetails(ctx, HttpStatusCode::Forbidden);
        return false;
    }
...
void processDelete(RequestData& req, const Client& client,
                   const RequestContext& ctx, RawResponse& rawResp)
{
    (void)req;
    (void)client;
    ...
    {
        DBG("[processDelete] Exception caught while deleting file: "
            << e.what());
        rawResp.addErrorDetails(ctx, HttpStatusCode::InternalServerError);
    }
}
void processPost(RequestData& req, const Client& client,
                 const RequestContext& ctx, RawResponse& rawResp,
                 CgiRequestResult& cgiResult)
{
    (void)client;
    ...
    if (!ctx.cgi_pass.empty())
    {
        ...
        std::string interpreter
            = getCgiPathFromUri(req.uri, ctx.cgi_pass, status);
        DBG("[processPost] CGI interpreter path: \""
            << interpreter << "\", status: " << static_cast<int>(status));
        ...
        if (status == HttpStatusCode::OK)
        {
            ...
            DBG("[processPost] interpreter detected, checking file "
                << requestedScript << " for validity...");

            if (!checkScriptValidity(requestedScript, rawResp, ctx))
                return;

            DBG("Executing CGI interpreter: " << interpreter << " for script: "
                                              << requestedScript);
    ...
    {
        DBG("[processPost] Upload store file does not exist in the file "
            "system");
        rawResp.addErrorDetails(ctx, HttpStatusCode::InternalServerError);
        return;
    }
...
void handleExternalRedirect(const RequestContext& ctx, std::string& reqUri,
                            RawResponse& rawResp)
{

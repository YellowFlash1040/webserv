...
void processGet(RequestData& req, const NetworkEndpoint& endpoint,
                const RequestContext& ctx, RawResponse& rawResp)
{
    ...
    if (!ctx.cgi_pass.empty())
    {
        ...
        DBG("[processGet] interpreter detected, checking file "
            << ctx.resolved_path << " for validity...");
        ...
        {
            DBG("[processGet] CGI Uri exists but is not executable: "
                << ctx.resolved_path);
        ...
        if (status == HttpStatusCode::OK)
        {
            DBG("Executing CGI interpreter: " << interpreter << " for script: "
                                              << ctx.resolved_path);
            CGIHandler::processCGI(req, endpoint, interpreter,
                                   ctx.resolved_path, rawResp);
    ...
    for (const auto& entry : cgi_pass)
    {
        ...
        if (entry.first == ext)
        {
            ...
            if (canonicalPath.string().compare(
                    0, std::string("/usr/bin/").size(), "/usr/bin/")
                != 0)
            {
                DBG("[getCgiPathFromUri] Interpreter path escapes /usr/bin/ → "
                    "BadGateway");
                outStatus = HttpStatusCode::BadGateway;
                return "";
            }
...
void processDelete(RequestData& req, const NetworkEndpoint& endpoint,
                   const RequestContext& ctx, RawResponse& rawResp)
{
    (void)req;
    (void)endpoint;
    ...
    {
        DBG("[processDelete] Failed to delete file: " << ctx.resolved_path);
...
void processPost(RequestData& req, const NetworkEndpoint& endpoint,
                 const RequestContext& ctx, RawResponse& rawResp)
{
    HttpStatusCode status;
    ...
    if (!ctx.cgi_pass.empty())
    {
        ...
        CGIHandler::processCGI(req, endpoint, interpreter, ctx.resolved_path,
                               rawResp);
...
void fillSuccessfulResponse(RawResponse& resp, const std::string& filePath)
{
    ...
    FileUtils::FileDeliveryMode mode
        = (fileSize <= MAX_IN_MEMORY_FILE_SIZE)

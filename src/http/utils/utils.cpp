...
void printReqContext(const RequestContext& ctx)
{
    ...
    for (std::map<HttpStatusCode, std::string>::const_iterator it
         = ctx.error_pages.begin();
    ...
    std::cout << "autoindex_enabled: "
              << (ctx.autoindex_enabled ? "true" : "false") << "\n";
    ...
    for (size_t i = 0; i < ctx.allowed_methods.size(); ++i)
        std::cout << "  "
                  << RawResponse::httpMethodToString(ctx.allowed_methods[i])
                  << "\n";

    std::cout << "redirection: " << static_cast<int>(ctx.redirection.statusCode)
              << " " << ctx.redirection.url << "\n";
